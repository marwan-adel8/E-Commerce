...
<div className="relative w-full h-64 md:h-96 rounded-2xl overflow-hidden mb-12 shadow-lg">
  <img
    src="/public/images/pexels-karolina-grabowska-5632346.jpg" // ← غيّر هذا المسار إلى صورة البانر الخاصة بك
    alt="Shop Banner"
      ...
      {categories.map((cat) => (
        <button
          ...
          className={`flex flex-col items-center justify-center border rounded-md py-6 bg-white text-gray-800 border-gray-300 hover:shadow transition
            ${cat.id === selectedCategory ? "border-indigo-500 shadow-lg ring-2 ring-indigo-300" : ""}`}
        >
          <img
            src={cat.image}
            alt={cat.label}
...
/>
              ...
              <p className="text-indigo-600 font-bold">{product.price.toFixed(2)} EGP</p>
